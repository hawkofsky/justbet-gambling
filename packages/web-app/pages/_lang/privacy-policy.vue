  ...
  <div class="main-content guides">
    ...
    <div class="container">
      <div class="py-5">
        <Header />
        <div class="mt-5">
          <h2 class="mb-5 semi-bold">{{ $t('privacy.title') }}</h2>
          <div class="row">
            <div class="col-12 col-lg-8">
              <div class="guide-bg">
                <div
                  v-for="(element, i) in $t('privacy.elements')"
                  :id="element.id"
                  :key="i"
                >
                  ...
                  <div v-for="(head, k) in element.heads" :key="k" class="mb-4">
                    <div :id="head.id" :key="k" class="mb-4">
                      <h5>{{ head.title }}</h5>
                    </div>
                    <p
                      v-for="(paragraph, j) in head.paragraphs"
                      :key="j"
                      class="mb-4"
                      v-html="paragraph"
                    />
                    <List v-if="head.list" :items="head.list" />
                    <div v-for="(sub, k) in head.subs" :id="sub.id" :key="k">
                      <div class="mb-4">
                        {{ sub.title }}
                      </div>
                      <p
                        v-for="(paragraph, j) in sub.paragraphs"
                        :key="j"
                        class="mb-4"
                        v-html="paragraph"
                      />
                      <List v-if="sub.list" :items="sub.list" />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div class="col-4 d-none d-lg-block">
              <div class="nav-content px-5 pt-4 pb-5">
                <b-nav v-b-scrollspy class="d-flex flex-column">
                  <template v-for="(element, i) in $t('privacy.elements')">
