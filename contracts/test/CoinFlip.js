...
contract('CoinFlip', function (accounts) {
    ...
    before(async () => {
        try {
            ...
            console.log('coinFlip', tronWeb.address.fromHex(coinFlip.address));
    ...
    it('user can win or lose, check all cases', async function () {
        ...
        while (countRetry > 0) {
            ...
            let result = await coinFlip.Flip(seed, side, ref, {
                callValue: bet,
    ...
    it('user can win or lose, check all cases TRC20', async function () {
        ...
        while (countRetry > 0) {
            ...
            let result = await coinFlip.FlipTRC20(seed, side, ref, USDT.address, bet, {
                callValue: 0,
                shouldPollResponse: true,
            });
    ...
    it('check user receive reward if win', async function () {
        ...
        const seed = 415151515151541;
        const bet = 15 * TRX;
        // wager * (gameRange - (houseEdge)) * (gameRange) / (5000) / (gameRange);
        const expectedReward = 29400000 - bet; // SUN
        while (countRetry > 0) {
            console.log("try make new bet");
            let userBalance = await tronWeb.trx.getBalance(accounts[0]);
            let transactionID = await coinFlip.Flip(seed, side, ref, {
                callValue: bet,
    ...
    it('check user receive reward if win TRC20', async function () {
        ...
        const bet = 15 * usdtDecimals;
        // wager * (gameRange - (houseEdge)) * (gameRange) / (5000) / (gameRange);
        const expectedReward = 29400000 - bet;
        while (countRetry > 0) {
            console.log("try make new bet");
            let userUSDTBalance = await balanceOfUsdt(accounts[0]);
            let result = await coinFlip.FlipTRC20(seed, side, ref, USDT.address, bet, {
    ...
    it('check bet limits', async function () {
        ...
        await assert.rejects(coinFlip.setMinBet(minBet, {
            shouldPollResponse: true,
            from: user,
        }), "Change limits can only owner");

        await assert.rejects(coinFlip.setMaxBet(maxBet, {
            shouldPollResponse: true,
            from: user,
        }), "Change limits can only owner");

        await coinFlip.setMinBet(minBet);
        await coinFlip.setMaxBet(maxBet);

        assert.strictEqual((await coinFlip.getMinBet()).toNumber(), minBet, "Wrong limit");
        assert.strictEqual((await coinFlip.getMaxBet()).toNumber(), maxBet, "Wrong limit");

        await assert.doesNotReject(coinFlip.Flip(seed, side, ref, {
            callValue: minBet,
            shouldPollResponse: true,
        ...
        await assert.rejects(coinFlip.Flip(seed, side, ref, {
            callValue: TRX,
            shouldPollResponse: true,
    ...
    it('check bet limits TRC20', async function () {
        ...
        await assert.rejects(coinFlip.setMinBetTRC20(USDT.address, minBet, {
            shouldPollResponse: true,
            from: user,
        }), "Change limits can only owner");

        await assert.rejects(coinFlip.setMaxBetTRC20(USDT.address, maxBet, {
            shouldPollResponse: true,
            from: user,
        }), "Change limits can only owner");

        await coinFlip.setMinBetTRC20(USDT.address, minBet);
        await coinFlip.setMaxBetTRC20(USDT.address, maxBet);
        //
        assert.strictEqual((await coinFlip.getMinBetTRC20(USDT.address)).toNumber(), minBet, "Wrong limit");
        assert.strictEqual((await coinFlip.getMaxBetTRC20(USDT.address)).toNumber(), maxBet, "Wrong limit");

        await assert.doesNotReject(coinFlip.FlipTRC20(seed, side, ref, USDT.address, minBet, {
            shouldPollResponse: true,
        }), "Bet in limit range must pass");
        ...
        await assert.doesNotReject(coinFlip.FlipTRC20(seed, side, ref, USDT.address, validBet, {
            shouldPollResponse: true,
        }), "Bet in limit range must pass");

        await assert.rejects(coinFlip.FlipTRC20(seed, side, ref, USDT.address, 5 * usdtDecimals, {
            shouldPollResponse: true,
        }), "Bet not in limit must reject");

        await assert.rejects(coinFlip.FlipTRC20(seed, side, ref, USDT.address, 2000 * usdtDecimals, {
            shouldPollResponse: true,
        }), "Bet not in limit must reject");
    });
});