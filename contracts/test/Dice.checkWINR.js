...
contract('Dice', function (accounts) {
    ...
    before(async () => {
        try {
            ...
            winr = await WINR.deployed();
            winrTronweb = await tronWeb.contract().at(WINR.address);
    ...
    it(`check user receive mined WINR's`, async () => {
        await winrTronweb.MiningData().watch((err, event) => {
            if (err) {
        ...
        const expectedReward = 37125000; // SUN
        while (countRetry > 0) {
            console.log("try make new bet");

            let userBalance = await winr.balanceOf(accounts[0]);
            ...
            const currentUserBalance = await winr.balanceOf(accounts[0]);
            console.log(`${currentUserBalance} >>> ${userBalance}`);
    ...
    it('check user receive reward if win', async function () {
        ...
        while (countRetry > 0) {
            ...
            if (result >= minFringe && result <= maxFringe) {
                console.log('user win');
                console.log(`${currentUserBalance} === ${userBalance} - ${fee} + ${expectedReward}`);
                console.log(`${currentUserBalance} === ${userBalance - fee + expectedReward}`);
